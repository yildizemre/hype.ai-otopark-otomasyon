...
def show_results():
    ...
    for i,ch in enumerate(char): #iterating over the characters
        ...
        img_con = cv2.convertScaleAbs(img, alpha=(255.0))
        rnd=str(random.random())+".jpg"
        cv2.imwrite(rnd, img_con)
    ...
    if len(indexes)>0:
        for i in indexes.flatten():
            ...
            if(label=="carLP"):
                ...
                cropped = org_img[y-5:y+h+5,x-5:x+w+5]
                resize_cropped = cv2.resize(
                    cropped, None, fx = 1.5, fy = 1.5, 
                    interpolation = cv2.INTER_AREA)
                
                char = segment_characters(resize_cropped)
